        ...
        <div className="space-y-5">
            ...
            <div className="flex gap-2">
              <div className="flex-1 relative">
                <input
                  ...
                  className="w-full bg-buncha-surface/30 border border-buncha-border rounded-xl px-4 py-3 text-sm text-buncha-text placeholder:text-buncha-text-muted/50 focus:outline-none focus:ring-2 focus:ring-buncha-accent/30 focus:border-buncha-accent transition-all disabled:opacity-50"
                />
                {isValidating ? (
              ...
              <button
                onClick={handlePaste}
                disabled={isDownloading}
                className="flex items-center gap-2 px-4 py-3 bg-buncha-surface/30 hover:bg-buncha-surface/50 border border-buncha-border rounded-xl text-sm text-buncha-text-muted hover:text-buncha-text transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {justCopied ? (
                  <Check className="w-4 h-4 text-green-500" />
                ) : (
          ...
          <div>
            <button
              ...
              disabled={isDownloading}
              className="w-full flex items-center justify-between p-3 rounded-xl border border-buncha-border hover:border-buncha-accent/30 hover:bg-buncha-surface/30 transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
            >
                    ...
                    {YOUTUBE_QUALITY_OPTIONS.map((quality) => {
                      ...
                      return (
                        <button
                          ...
                          disabled={isDownloading}
                          className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-all cursor-pointer disabled:opacity-50 ${
                            isSelected
                    ...
                    {YOUTUBE_DOWNLOAD_MODES.map((mode) => {
                      ...
                      return (
                        <button
                          ...
                          disabled={isDownloading}
                          className={`flex flex-col items-center gap-1.5 px-3 py-3 rounded-lg text-sm transition-all cursor-pointer disabled:opacity-50 ${
                            isSelected
                              ? "bg-buncha-accent text-white"
                              : "bg-buncha-surface/50 text-buncha-text-muted hover:bg-buncha-surface hover:text-buncha-text border border-buncha-border"
                          }`}
          ...
          <div className="flex items-center justify-between pt-2">
            <button
              onClick={onReset}
              disabled={isDownloading}
              className="flex items-center gap-2 px-4 py-2 text-sm text-buncha-text-muted hover:text-buncha-text transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
            >
            ...
            <div className="flex items-center gap-2">
              ...
              {isDownloading && (
                <button
                  onClick={onCancel}
                  className="flex items-center justify-center w-10 h-10 bg-red-500/10 hover:bg-red-500/20 border border-red-500/20 rounded-xl text-red-400 hover:text-red-300 transition-all cursor-pointer"
              ...
              <button
                onClick={isComplete ? onOpenFolder : onDownload}
                disabled={!canDownload && !isDownloading && !isComplete}
                className={`relative flex items-center justify-center gap-2 px-6 py-3 font-medium rounded-xl transition-all overflow-hidden min-w-[140px] ${
                  isComplete
                    ? "bg-green-500 text-white cursor-pointer"
                    : isDownloading
                      ? "bg-buncha-surface/50 text-buncha-text cursor-not-allowed"
                      : canDownload
        ...
        <div className="flex items-center justify-between text-xs text-buncha-text-muted">
          <span>
            {isComplete
              ...
              : isDownloading
                ? `Downloading: ${options.mode === "audio_only" ? "Audio" : options.mode === "video_only" ? "Video" : "Video + Audio"} @ ${options.quality.toUpperCase()}`
                : parsedUrl?.isValid
